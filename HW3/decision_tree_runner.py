...
if __name__ == "__main__":
    ...
    dataPath = "albert/hw3_train.txt"
    dataPath2 = "albert/hw3_test.txt"
    if len(sys.argv) == 2:
        dataPath = sys.argv[1]
    if not os.path.isfile(dataPath):
        ...
        #sc.stop()
        #usage()
    trainData = MLUtils.loadLibSVMFile(sc, dataPath)
    testData = MLUtils.loadLibSVMFile(sc, dataPath2)

    # Re-index class labels if needed.
    (reindexedData, origToNewLabels) = reindexClassLabels(trainData)
    numClasses = len(origToNewLabels)

    # Train a classifier.
    categoricalFeaturesInfo = {}  # no categorical features
    #model = DecisionTree.trainClassifier(reindexedData, numClasses=numClasses,
    #                                     categoricalFeaturesInfo=categoricalFeaturesInfo, impurity='gini')
    itr = 10
    for i in range(itr):
        model = RandomForest.trainClassifier(reindexedData, numClasses=numClasses,
                                             categoricalFeaturesInfo=categoricalFeaturesInfo, 
                                             numTrees=300, impurity='gini', maxDepth=1)
        # Print learned tree and stats.
        predictions = model.predict(trainData.map(lambda x: x.features))
        labelsAndPredictions = trainData.map(lambda lp: lp.label).zip(predictions)
        testErr = labelsAndPredictions.filter(lambda (v, p): v != p).count() / float(trainData.count())
        print(str(i) + ' Test Error = ' + str(testErr))
